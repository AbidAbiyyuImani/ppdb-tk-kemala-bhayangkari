<x-layout>
  <x-card class="flex flex-col">
    <div class="flex flex-col sm:flex-row items-center sm:justify-between py-4">
      <h2 class="text-lg sm:text-xl font-semibold text-gray-800">Data Dokumen</h2>
      <div class="flex space-x-2 items-center">
        <a href="{{ route('show.master-data') }}">
          <x-button color="bg-gray-600">Kembali</x-button>
        </a>
        <a href="{{ route('show.create.data-document') }}">
          <x-button color="bg-blue-600">Tambah</x-button>
        </a>
      </div>
    </div>

    <x-forms.filter :action="route('show.data-document')" />
        ...
        <tbody class="divide-y divide-gray-200">
          @forelse ($dokumen as $data)
            <tr>
              ...
              <td class="space-x-2">
                <a href="{{ asset('storage/' . $data->path_dokumen) }}" target="_blank">
                  <x-button color="bg-blue-600">Lihat</x-button>
                </a>
                @if ($data->deleted_at)    
                  <form action="{{ route('restore.data-document', [$data->pendaftaran?->slug ?? 'default', $data->slug]) }}" method="POST" class="inline">
                    @csrf @method('PATCH')
                    <x-button type="submit" color="bg-orange-500" onclick="return confirm('Pulihkan data dokumen ini?')">Pulihkan</x-button>
                  </form>
                @else
                  <a href="{{ route('show.update.data-document', [$data->pendaftaran?->slug ?? 'default', $data->slug]) }}">
                    <x-button color="bg-yellow-500">Ubah</x-button>
                  </a>
                  <form action="{{ route('destroy.data-document', [$data->pendaftaran?->slug ?? 'default', $data->slug]) }}" method="POST" class="inline">
                    @csrf @method('DELETE')
                    <x-button type="submit" color="bg-red-500" onclick="return confirm('Apakah anda yakin ingin menghapus data dokumen ini?')" :disabled="$data->id_dokumen < 4 ? true : false">Hapus</x-button>
                  </form>
                @endif
              </td>
            </tr>
      ...
      @forelse ($dokumen as $data)
        <div class="rounded-md shadow-sm">
          <button class="accordion-toggle w-full space-x-0.5 px-4 py-3 bg-gray-100 rounded-md text-left font-semibold text-sm flex justify-between items-center">
            <span>{{ $data->pendaftaran->nama_panggilan ?? '-' }} | {{ $data->nama_dokumen }}</span>
          ...
          <div class="accordion-content bg-gray-50 px-4 py-3 text-sm space-y-1">
            ...
            <p><strong>Nama Dokumen:</strong> {{ $data->nama_dokumen }}</p>
            <p><strong>Tanggal Upload:</strong> {{ $data->created_at->diffForHumans() }}</p>
