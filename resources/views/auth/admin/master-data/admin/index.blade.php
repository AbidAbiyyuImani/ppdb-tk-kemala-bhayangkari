<x-layout>
  <x-card class="flex flex-col">
    <div class="flex flex-col sm:flex-row items-center sm:justify-between py-4">
      <h2 class="text-lg sm:text-xl font-semibold text-gray-800">Data Admin</h2>
      <div class="flex space-x-2 items-center">
        <a href="{{ route('show.master-data') }}">
          <x-button color="bg-gray-600">Kembali</x-button>
        </a>
        <a href="{{ route('show.create.data-admin') }}">
          <x-button color="bg-blue-600">Tambah</x-button>
        </a>
      </div>
    </div>

    <x-forms.filter :action="route('show.data-admin')" />
        ...
        <tbody class="divide-y divide-gray-200">
          @forelse ($admin as $data)
            <tr>
              ...
              <td class="space-x-2">
                @if ($data->deleted_at)    
                  <form action="{{ route('restore.data-admin', [$data->slug]) }}" method="POST" class="inline">
                    @csrf @method('PATCH')
                    <x-button type="submit" color="bg-orange-500" onclick="return confirm('Pulihkan data admin ini?')">Pulihkan</x-button>
                  </form>
                @else
                  <a href="{{ route('show.update.data-admin', [$data->slug]) }}">
                    <x-button color="bg-yellow-500">Ubah</x-button>
                  </a>
                  <form action="{{ route('destroy.data-admin', [$data->slug]) }}" method="POST" class="inline">
                    @csrf @method('DELETE')
                    <x-button type="submit" color="bg-red-500" onclick="return confirm('Apakah anda yakin ingin menghapus data admin ini?')">Hapus</x-button>
                  </form>
                @endif
              </td>
            </tr>
