  ...
  <x-card class="flex flex-col">
    <div class="flex flex-col sm:flex-row items-center sm:justify-between py-4">
      ...
      <div class="flex space-x-2 items-center">
        ...
        <a href="{{ route('show.create.data-registration') }}">
          <x-button color="bg-blue-600">Tambah</x-button>
        </a>
      </div>
    </div>

    <x-forms.filter :action="route('show.data-registration')" />
        ...
        <tbody class="divide-y divide-gray-200">
          @forelse ($pendaftaran as $data)
            <tr>
              ...
              <td class="space-x-2">
                @if ($data->deleted_at)    
                  <form action="{{ route('restore.data-registration', [$data->user->slug, $data->slug]) }}" method="POST" class="inline">
                    @csrf @method('PATCH')
                    <x-button type="submit" color="bg-orange-500" onclick="return confirm('Pulihkan data pendaftaran ini?')">Pulihkan</x-button>
                  </form>
                @else
                  <a href="{{ route('show.update.data-registration', [$data->user->slug, $data->slug]) }}">
                    <x-button color="bg-yellow-500">Ubah</x-button>
                  </a>
                  <form action="{{ route('destroy.data-registration', [$data->user->slug, $data->slug]) }}" method="POST" class="inline">
                    @csrf @method('DELETE')
                    <x-button type="submit" color="bg-red-500" onclick="return confirm('Apakah anda yakin ingin menghapus data pendaftaran ini?')">Hapus</x-button>
                  </form>
                @endif
              </td>
            </tr>
